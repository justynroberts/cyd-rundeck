...
bool display::factoryResetPrompt(uint32_t holdMs) {
    ...
    tft.setTextSize(1);
    tft.setTextColor(0xC638, TFT_BLACK);
    tft.drawString("Hold to wipe WiFi + Rundeck token", SCREEN_WIDTH / 2, 92);
    tft.drawString("Release to cancel", SCREEN_WIDTH / 2, 108);

    // Rundeck blue progress bar
    const uint16_t RD_BLUE = 0x451F;  // ~#4a90e2 in RGB565
    const int barW = 220, barH = 14, barX = (SCREEN_WIDTH - barW) / 2, barY = 150;
    tft.drawRect(barX, barY, barW, barH, RD_BLUE);
    ...
    while (millis() - start < holdMs) {
        ...
        if (fillW != last) {
            tft.fillRect(barX + 1, barY + 1, fillW, barH - 2, RD_BLUE);
            last = fillW;
        }
        delay(15);
    }
    tft.fillRect(barX + 1, barY + 1, barW - 2, barH - 2, RD_BLUE);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.setTextSize(2);
